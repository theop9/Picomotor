...
class picomotor:
    ...
    def get_all_positions(self):
        ...
        for addr in [1,2]:
            for axis in [1,2,3]:
                nPosition = 0
                nReturn = cmdLib8742.GetPosition(strDeviceKey, 1, 1, nPosition)
                if nReturn:
                    positions[addr-1, axis-1] = nPosition
        return positions
...
indexMode = 0
Mode = np.array(['mode1 (fibres indépendentes)', 'mode2 (en bloc)'])
...
try:
    while running:
        ...
        # pygame.draw.circle(screen, "red", (fibre1_pos.x, fibre1_pos.y), 4)
        # pygame.draw.circle(screen, "blue", (fibre2_pos.x, fibre2_pos.y), 4)
        ...
        if indexMode == 0:              
            ...
            # Représentation de la position des fibres sur l'écran
            # x1, y1, z1 = get_position(1)
            # x2, y2, z2 = get_position(2)
            # fibre1_pos.x = x1
            # fibre1_pos.y = y1
            # fibre2_pos.x = x2
            # fibre2_pos.y = y2
        ...
        text_print.tprint(screen, f"Fibre 1 : x = {fibre1_pos.x:.0f}, y = {-fibre1_pos.y:.0f}, z = {fibre1_pos.z:.0f}")
        text_print.tprint(screen, f"Fibre 2 : x = {fibre2_pos.x:.0f}, y = {-fibre2_pos.y:.0f}, z = {fibre2_pos.z:.0f}")
